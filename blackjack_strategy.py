...
def black_jack_suggestion(hand, dealer_card):
    hard_count=sum(hand) # Hard count = No Aces, or Ace == 1, not 11
    cards = len(hand) 
    aces_count = 1 * (1 in hand) # Counts number of Aces in card
    soft_count = hard_count # temporary value for soft_hand
    
    if cards < 2:
        return "Need more cards"
    ...
    if hard_count == soft_count:
        ...
        if hard_count == 10 and dealer_card not in [1,10]:
            return "Double"
        if hard_count == 9 and dealer_card not in [2,1,7,8,9,10]:
            return "Double"
        if hard_count <= 11:
            return "Hit"
    ...
    if hard_count != soft_count:
        ...
        if soft_count == 18:
            ...
            if dealer_card not in [2,1,7,8,9,10]:
